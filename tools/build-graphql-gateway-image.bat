@echo off
REM Build only the graphql-gateway Docker image using the runtime Dockerfile (Windows batch)
REM Usage: build-graphql-gateway-image.bat [--jar <path>] [--dockerfile <path>] [--tag <image:tag>] [--no-cache]

setlocal EnableDelayedExpansion

set "JAR=parent/graphql-gateway/boot/target/graphql-gateway-boot-1.5.0.jar"
...
if not exist "%JAR_PATH%" (
  ...
  echo You can build it with: mvn -pl parent/graphql-gateway/boot -am -DskipTests package
)
